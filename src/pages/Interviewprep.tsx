...
const Button: React.FC<ButtonProps> = ({
  variant = 'solid', className = '', disabled, children, ...props
}) => {
  const base =
    'inline-flex items-center justify-center px-5 py-3 rounded-xl text-sm font-semibold transition-all duration-200 active:scale-95 disabled:opacity-60 disabled:cursor-not-allowed';
...
// ─────────────────────────────────────────────────────────────────────────────
// Inline Textarea (replaces shadcn)
// ─────────────────────────────────────────────────────────────────────────────
...
export default function InterviewPrep() {
  ...
  // ✅ Reads the job passed from JobGrid: navigate('/interview-prep', { state: { job } })
  ...
  // ── Generate 5 interview questions via Claude ────────────────────────────────
  const generateQuestions = async (): Promise<void> => {
    setLoading(true);
    setError(null);
    try {
      const raw = await callClaude(
        `Generate 5 realistic interview questions for a ${job!.title} position at ${job!.company}.
...
Respond ONLY with valid JSON — no explanation, no markdown:
{
  ...
  "rating": <integer 1-10>,
  "suggested_answer": "a strong example answer"
}`
      );
              ...
              {showFeedback && feedback && (
                ...
                >
                  ...
                  <div className="bg-white rounded-2xl border border-black/[0.06] p-6 text-center shadow-sm">
                    <p className="text-xs font-bold text-[#1A1A1A]/30 uppercase tracking-widest mb-2">
                      Your Score
                    </p>
                    <p className="text-5xl font-bold text-[#C5A059]">
                      {feedback.rating}
                      <span className="text-2xl text-[#1A1A1A]/20">/10</span>
                    </p>
                  </div>
                ...
                <div className="space-y-2.5">
                  {allAnswers.map((item, i) => (
                    <div key={i} className="flex items-center justify-between text-xs gap-4">
                      ...
                      <span className="font-bold text-[#C5A059] flex-shrink-0">
                        {item.feedback.rating}/10
                      </span>
                    </div>
                  ))}
                  <div className="pt-3 border-t border-black/[0.05] flex items-center justify-between">
                    ...
                    <span className="text-lg font-bold text-[#C5A059]">
                      {(
                        allAnswers.reduce((sum, a) => sum + a.feedback.rating, 0) / allAnswers.length
                      ).toFixed(1)}/10
                    </span>
                  </div>
                </div>
