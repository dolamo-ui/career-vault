...
export const AuthPage: React.FC<AuthPageProps> = ({ mode = 'login' }) => {
  ...
  const resetForm = () => { setMessage(''); setEmail(''); setPassword(''); setConfirmPassword(''); };

  // ── Countdown helper for resend button ──
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Force refresh to get latest emailVerified status from Firebase
      await reload(user);
      ...
      setTimeout(() => { setIsLoading(false); navigate('/home'); }, 800);
    } catch (err) { setIsLoading(false); setMessage(friendlyError(err as AuthError)); }
  };
  ...
  // ── Poll / manually check if user has verified ──
  const handleCheckVerified = async () => {
    setCheckingVerification(true);
    try {
      const user = auth.currentUser;
      if (user) {
        // Force refresh — this is the important trick
        await reload(user);
        if (user.emailVerified) {
          localStorage.setItem('userId', user.uid);
          localStorage.setItem('userEmail', user.email ?? '');
          ...
          setMessage('⚠️ Email not verified yet. Please click the link in your inbox.');
        }
      }
    } catch {
      setMessage('❌ Could not check verification status. Try logging in.');
    }
            ...
            <>
              ...
              <form onSubmit={handleForgotPassword} className="space-y-5">
                ...
                <button
                  ...
                  className="w-full py-4 bg-[#1A1A1A] text-white rounded-xl font-bold shadow-xl shadow-black/10 hover:bg-[#333] hover:-translate-y-0.5 transition-all duration-300 active:scale-95 disabled:opacity-70 flex items-center justify-center group"
                >
                  {resetLoading
    ...
    return (
      <div className="min-h-screen bg-[#FDFCFB] flex items-center justify-center p-6">
        <div className="max-w-md w-full text-center">
          ...
          <p className="font-bold text-[#1A1A1A] mb-6 text-lg">{verificationEmail}</p>
          <p className="text-[#1A1A1A]/40 text-sm mb-10">
          ...
          {/* Message */}
          {message && (
            <div className={`p-3 rounded-xl text-sm font-semibold mb-6 ${
              message.includes('✅')
                ? 'bg-emerald-50 text-emerald-700 border border-emerald-200'
                : 'bg-rose-50 text-rose-700 border border-rose-200'
            }`}>
          ...
          <button
            ...
            className="w-full py-4 bg-[#1A1A1A] text-white rounded-xl font-bold shadow-xl shadow-black/10 hover:bg-[#333] hover:-translate-y-0.5 transition-all duration-300 active:scale-95 disabled:opacity-70 flex items-center justify-center mb-4"
          >
            {checkingVerification
              ? <div className="w-6 h-6 border-2 border-white/30 border-t-white rounded-full animate-spin" />
              : <><MailCheck className="w-5 h-5 mr-2" /> I've verified my email</>
            }
          </button>
          ...
          <button
            ...
            className="text-sm font-semibold text-[#C5A059] hover:text-[#B8903F] transition-colors disabled:text-[#1A1A1A]/30 disabled:cursor-not-allowed"
          >
            {resendCooldown > 0
          ...
          <div className="mt-8">
            <button
              onClick={() => { setVerificationSent(false); setIsLogin(true); resetForm(); }}
              className="flex items-center justify-center mx-auto text-[#1A1A1A]/40 text-sm font-semibold hover:text-[#1A1A1A] transition-colors"
            >
              <ArrowLeft className="w-4 h-4 mr-1" /> Back to login
            </button>
          </div>
          ...
          <div className="max-w-md w-full mx-auto">
            ...
            <div className="bg-black/[0.04] p-1.5 rounded-2xl mb-8 flex relative">
              ...
              <button
                ...
                className={`flex-1 py-2.5 text-sm font-bold relative z-10 transition-colors duration-200 ${
                  isLogin ? 'text-[#1A1A1A]' : 'text-[#1A1A1A]/40'
                }`}
              >
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div>
                ...
                {/* Password strength checklist — only on register */}
              ...
              {/* Unverified email — show resend option inline on login screen */}
